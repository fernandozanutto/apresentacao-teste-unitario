...
import {DateAdapter, APLS_DATE_LOCALE, NativeDateAdapter, NativeDateModule} from '.';
...
describe('NativeDateAdapter with APLS_DATE_LOCALE override', () => {
  let adapter: NativeDateAdapter;

  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [NativeDateModule],
      providers: [{provide: APLS_DATE_LOCALE, useValue: 'da-DK'}]
    }).compileComponents();
  }));
  ...
  it('should take the default locale id from the APLS_DATE_LOCALE injection token', () => {
...
describe('NativeDateAdapter with LOCALE_ID override', () => {
  ...
  it('should cascade locale id from the LOCALE_ID injection token to APLS_DATE_LOCALE', () => {
    const expectedValue = SUPPORTS_INTL ?
        ['søndag', 'mandag', 'tirsdag', 'onsdag', 'torsdag', 'fredag', 'lørdag'] :
        ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
