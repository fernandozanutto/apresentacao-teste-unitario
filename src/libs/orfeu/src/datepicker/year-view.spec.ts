...
import {dispatchFakeEvent, dispatchKeyboardEvent} from '@apollus-ngx/orfeu';
...
describe('AplsYearView', () => {
  ...
  describe('standard year view', () => {
    ...
    beforeEach(() => {
      ...
      let yearViewDebugElement = fixture.debugElement.query(By.directive(AplsYearView));
      yearViewNativeElement = yearViewDebugElement.nativeElement;
      testComponent = fixture.componentInstance;
    });

    it('has correct year label', () => {
      let labelEl = yearViewNativeElement.querySelector('.apls-calendar-body-label')!;
      expect(labelEl.innerHTML.trim()).toBe('2017');
    });

    it('has 12 months', () => {
      let cellEls = yearViewNativeElement.querySelectorAll('.apls-calendar-body-cell')!;
      expect(cellEls.length).toBe(12);
    });

    it('shows selected month if in same year', () => {
      let selectedEl = yearViewNativeElement.querySelector('.apls-calendar-body-selected')!;
    ...
    it('should mark active date', () => {
      ...
      expect(cellEls[0].classList).toContain('apls-calendar-body-active');
    });
    ...
    describe('a11y', () => {
      describe('calendar body', () => {
        ...
        it('should go to first month of the year on aplicacao press', () => {
          calendarInstance.yearView.activeDate = new Date(2017, SEP, 30);
          fixture.detectChanges();
  ...
  describe('year view with date filter', () => {
    ...
    beforeEach(() => {
      ...
      const yearViewDebugElement = fixture.debugElement.query(By.directive(AplsYearView));
      yearViewNativeElement = yearViewDebugElement.nativeElement;
    });

    it('should disable months with no enabled days', () => {
      const cells = yearViewNativeElement.querySelectorAll('.apls-calendar-body-cell');
...
@Component({
  template: `
    <apls-year-view [activeDate]="date" [(selected)]="selected"
                   (monthSelected)="selectedMonth=$event"></apls-year-view>`
})
class StandardYearView {
  ...
  @ViewChild(AplsYearView) yearView: AplsYearView<Date>;
}


@Component({
  template: `<apls-year-view [activeDate]="activeDate" [dateFilter]="dateFilter"></apls-year-view>`
})
