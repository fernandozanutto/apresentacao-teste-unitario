...
import {AplsCalendarBody, AplsCalendarCell} from './calendar-body';
import {By} from '@angular/platform-browser';


describe('AplsCalendarBody', () => {
  beforeEach(async(() => {
  ...
  describe('standard calendar body', () => {
    ...
    let refreshElementLists = () => {
      ...
      labelEls = calendarBodyNativeElement.querySelectorAll('.apls-calendar-body-label');
      cellEls = calendarBodyNativeElement.querySelectorAll('.apls-calendar-body-cell');
    };
    ...
    beforeEach(() => {
      ...
      let calendarBodyDebugElement = fixture.debugElement.query(By.directive(AplsCalendarBody));
    ...
    it('highlights today', () => {
      let todayCell = calendarBodyNativeElement.querySelector('.apls-calendar-body-today')!;
      expect(todayCell).not.toBeNull();
      expect(todayCell.innerHTML.trim()).toBe('3');
    });

    it('highlights selected', () => {
      let selectedCell = calendarBodyNativeElement.querySelector('.apls-calendar-body-selected')!;
    ...
    it('places label in first row if space is available', () => {
      ...
      expect(rowEls[0].firstElementChild!.classList)
          .toContain('apls-calendar-body-label', 'first cell should be the label');
      expect(labelEls[0].getAttribute('colspan')).toBe('3');
    });

    it('cell should be selected on click', () => {
      let todayElement =
          calendarBodyNativeElement.querySelector('.apls-calendar-body-today') as HTMLElement;
      todayElement.click();
      fixture.detectChanges();

      expect(todayElement.classList)
          .toContain('apls-calendar-body-selected', 'today should be selected');
    });

    it('should mark active date', () => {
      expect((cellEls[10] as HTMLElement).innerText.trim()).toBe('11');
      expect(cellEls[10].classList).toContain('apls-calendar-body-active');
    });
  });
...
@Component({
  template: `<table apls-calendar-body
                    [label]="label"
                    [rows]="rows"
...
function createCell(value: number) {
  return new AplsCalendarCell(value, `${value}`, `${value}-label`, true);
}
