...
import {JAN, AplsNativeDateModule} from '../core';
import {AplsCalendarBody} from './calendar-body';
import {AplsMultiYearView, yearsPerPage, yearsPerRow} from './multi-year-view';
import {dispatchKeyboardEvent, dispatchFakeEvent} from '@apollus-ngx/orfeu';

describe('AplsMultiYearView', () => {
  ...
  describe('standard multi-year view', () => {
    ...
    beforeEach(() => {
      ...
      let multiYearViewDebugElement = fixture.debugElement.query(By.directive(AplsMultiYearView));
      multiYearViewNativeElement = multiYearViewDebugElement.nativeElement;
      testComponent = fixture.componentInstance;
    });

    it('has correct number of years', () => {
      let cellEls = multiYearViewNativeElement.querySelectorAll('.apls-calendar-body-cell')!;
      expect(cellEls.length).toBe(yearsPerPage);
    });

    it('shows selected year if in same range', () => {
      let selectedEl = multiYearViewNativeElement.querySelector('.apls-calendar-body-selected')!;
    ...
    it('should mark active date', () => {
      ...
      expect(cellEls[1].classList).toContain('apls-calendar-body-active');
    });
    ...
    describe('a11y', () => {
      describe('calendar body', () => {
        ...
        beforeEach(() => {
          ...
          calendarBodyEl =
            fixture.debugElement.nativeElement.querySelector('.apls-calendar-body') as HTMLElement;
        ...
        it('should go to first year in current range on aplicacao press', () => {
  ...
  describe('multi year view with date filter', () => {
    ...
    beforeEach(() => {
      ...
      const multiYearViewDebugElement = fixture.debugElement.query(By.directive(AplsMultiYearView));
      multiYearViewNativeElement = multiYearViewDebugElement.nativeElement;
    });

    it('should disablex years with no enabled days', () => {
      const cells = multiYearViewNativeElement.querySelectorAll('.apls-calendar-body-cell');
      expect(cells[0].classList).not.toContain('apls-calendar-body-disabled');
      expect(cells[1].classList).toContain('apls-calendar-body-disabled');
    });
  });
});
...
@Component({
  template: `
    <apls-multi-year-view [activeDate]="date" [(selected)]="selected"
                         (yearSelected)="selectedYear=$event"></apls-multi-year-view>`
})
class StandardMultiYearView {
  ...
  @ViewChild(AplsMultiYearView) multiYearView: AplsMultiYearView<Date>;
}

@Component({
  template: `
    <apls-multi-year-view [activeDate]="activeDate" [dateFilter]="dateFilter"></apls-multi-year-view>
    `
})
